...
static switch_xml_t xml_amqp_fetch(const char *section, const char *tag_name, const char *key_name, const char *key_value, switch_event_t *params,
                                                                  void *user_data)
{
        ...
        // strcpy(amqp_message->routing_key, switch_str_nil(section));
        if (profile->running && profile->conn_active) {
			    ...
			    if (conn->next == NULL && i < profile->max_temp_conn) {
			    	conn->next = switch_core_alloc(profile->pool, sizeof(mod_amqp_aux_connection_t));
                                status = mod_amqp_aux_connection_open(profile->conn_active, &(conn->next), profile->name, profile->custom_attr, profile->reply_exchange);
			...
			amqp_maybe_release_buffers(conn_tmp->state);
				...
				char *fs_resp_id = NULL;
					...
					if (strcmp(fs_resp_id, uuid_str) == 0) {
                                        ...
                                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Got wrong message, Trying the next one... %s vs %s\n", fs_resp_id, uuid_str);
					continue;
                                }
                        }

                        if (res.reply_type != AMQP_RESPONSE_NORMAL || !(xml = switch_xml_parse_str_dynamic((char *)envelope.message.body.bytes, SWITCH_TRUE))) {
...
switch_status_t mod_amqp_xml_handler_destroy(mod_amqp_xml_handler_profile_t **prof)
{
	...
	if (profile->xml_handler_thread) {
		switch_thread_join(&status, profile->xml_handler_thread);
	}
...
switch_status_t mod_amqp_xml_handler_create(char *name, switch_xml_t cfg)
{
	...
	profile->reply_exchange = reply_exchange ? reply_exchange : switch_core_strdup(profile->pool, "TAP.XML_handler_reply");
	profile->reply_exchange_type = reply_exchange_type ? reply_exchange_type : switch_core_strdup(profile->pool, "direct");
	...
	profile->conn_active = NULL;
	/* We are not going to open the xml_handler queue connection on create, but instead wait for the running thread to open it */
	...
	if (switch_thread_create(&profile->xml_handler_thread, thd_attr, mod_amqp_xml_handler_thread, profile, profile->pool)) {
    ...
    if (!profile->conn_active) {
      ...
      if ( status	== SWITCH_STATUS_SUCCESS ) {
		  ...
		  for (conn_aux = profile->conn_aux; conn_aux; conn_aux=conn_next) {
                      mod_amqp_aux_connection_open(profile->conn_active, &(conn_aux), profile->name, profile->custom_attr, profile->reply_exchange);
		  ...
		  amqp_exchange_declare(profile->conn_active->state, 1,
								...
								amqp_empty_table);
		amqp_exchange_declare(profile->conn_active->state, 1,
								amqp_cstring_bytes(profile->reply_exchange),
								amqp_cstring_bytes(profile->reply_exchange_type),
								passive,
								durable,
								profile->exchange_auto_delete,
								0,
